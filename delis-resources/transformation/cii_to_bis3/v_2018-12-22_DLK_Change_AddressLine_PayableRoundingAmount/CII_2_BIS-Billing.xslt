...
******************************************************************************************************************
        ...
        Repository path:    $HeadURL: https://svn.softwareborsen.dk/nemhandelinternal/Dokumentstandarder/PEPPOL/BIS3/Konvertering/CII_2_BIS3/CII_2_BIS-Billing.xslt $
        File version:       $Revision: 34856 $
        Last changed by:    $Author: JespermySup $
        Last changed date:  $Date: 2018-03-15 11:45:25 +0100 (to, 15 mar. 2018) $
...
<xsl:stylesheet version="2.0"
	...
	<xsl:template match="/rsm:CrossIndustryInvoice">
		...
		<xsl:choose>
			<xsl:when test="rsm:ExchangedDocument/ram:TypeCode = ('380','393','82','80','84','395','575','623','780')">
				<xsl:call-template name="InvoiceMapping"/>
			</xsl:when>
			<xsl:when test="rsm:ExchangedDocument/ram:TypeCode = ('381','396','81','83','532')">
	...
	<xsl:variable name="InvoiceTypeCode" select="rsm:ExchangedDocument/ram:TypeCode"/>
				...
				<xsl:if test="rsm:SupplyChainTradeTransaction/ram:ApplicableHeaderTradeSettlement/ram:SpecifiedTradeSettlementHeaderMonetarySummation/ram:TaxTotalAmount">
					<cbc:TaxAmount>
						<xsl:choose>
							...
							<xsl:otherwise><xsl:attribute name="currencyID" select="$HeaderCurrency"/></xsl:otherwise>
						</xsl:choose>
					<xsl:value-of select="rsm:SupplyChainTradeTransaction/ram:ApplicableHeaderTradeSettlement/ram:SpecifiedTradeSettlementHeaderMonetarySummation/ram:TaxTotalAmount[@currencyID=../../../ram:ApplicableHeaderTradeSettlement/ram:InvoiceCurrencyCode]"/></cbc:TaxAmount>
				</xsl:if>
	...
	<xsl:template match="rsm:ExchangedDocument/ram:IncludedNote">
		<cbc:Note><xsl:value-of select="ram:Content"/></cbc:Note>
	</xsl:template>
	...
	<xsl:template match="rsm:SupplyChainTradeTransaction/ram:ApplicableHeaderTradeAgreement/ram:BuyerOrderReferencedDocument">
		<cac:OrderReference>
			<cbc:ID><xsl:value-of select="ram:IssuerAssignedID"/></cbc:ID>
			<xsl:if test="../ram:SellerOrderReferencedDocument/ram:IssuerAssignedID">
				<cbc:SalesOrderID><xsl:value-of select="../ram:SellerOrderReferencedDocument/ram:IssuerAssignedID"/></cbc:SalesOrderID>
			</xsl:if>
		</cac:OrderReference>
	</xsl:template>
		...
		<cac:BillingReference>
			<cac:InvoiceDocumentReference>
				...
				<xsl:if test="ram:FormattedIssueDateTime/qdt:DateTimeString">
					<cbc:IssueDate><xsl:value-of select="substring(ram:FormattedIssueDateTime/qdt:DateTimeString, 1,4)"/>-<xsl:value-of select="substring(ram:FormattedIssueDateTime/qdt:DateTimeString, 5,2)"/>-<xsl:value-of select="substring(ram:FormattedIssueDateTime/qdt:DateTimeString, 7,2)"></xsl:value-of>
					</cbc:IssueDate>
				</xsl:if>
			</cac:InvoiceDocumentReference>
		</cac:BillingReference>
	...
	<xsl:template match="rsm:SupplyChainTradeTransaction/ram:ApplicableHeaderTradeAgreement[ram:AdditionalReferencedDocument/ram:TypeCode = '50']">
		<cac:OriginatorDocumentReference>
			<cbc:ID><xsl:value-of select="ram:AdditionalReferencedDocument[ram:TypeCode = '50']/ram:IssuerAssignedID"/></cbc:ID>
		</cac:OriginatorDocumentReference>
	</xsl:template>
	...
	<xsl:if test="../ram:AdditionalReferencedDocument[ram:TypeCode != '50']">
		...
		<xsl:variable name="Type" select="ram:TypeCode"/>
			<cac:AdditionalDocumentReference>
				<cbc:ID><xsl:value-of select="ram:IssuerAssignedID"/></cbc:ID>
				<xsl:if test="$Type"><cbc:DocumentTypeCode><xsl:value-of select="$Type"/></cbc:DocumentTypeCode></xsl:if>
				<xsl:if test="string(ram:Name)">
					<cbc:DocumentDescription><xsl:value-of select="ram:Name"/></cbc:DocumentDescription>
				</xsl:if>
				<xsl:if test="ram:AttachmentBinaryObject">
					<cac:Attachment>
						<cbc:EmbeddedDocumentBinaryObject mimeCode = "{$MimeCode}" filename = "{$fileName}"><xsl:value-of select="ram:AttachmentBinaryObject"/></cbc:EmbeddedDocumentBinaryObject>
						<xsl:if test="string(ram:URIID)">
							<cac:ExternalReference>
								<cbc:URI><xsl:value-of select="ram:URIID"/></cbc:URI>
							</cac:ExternalReference>
						</xsl:if>
					</cac:Attachment>
				</xsl:if>
	...
	<xsl:template match="rsm:SupplyChainTradeTransaction/ram:ApplicableHeaderTradeAgreement/ram:SpecifiedProcuringProject">
		<cac:ProjectReference>
			<cbc:ID><xsl:value-of select="ram:ID"/></cbc:ID>
		</cac:ProjectReference>
	</xsl:template>
	...
	<!--Varibles for the supplier class-->
		<cac:AccountingSupplierParty>
			<cac:Party>
				...
				<xsl:if test="string(ram:Name)">
					<cac:PartyName>
						<cbc:Name><xsl:value-of select="ram:Name"/></cbc:Name>
					</cac:PartyName> 
				</xsl:if>
				...
				<cac:PostalAddress>
					<xsl:if test="string(ram:PostalTradeAddress/ram:LineOne)">					
						<cbc:StreetName><xsl:value-of select="ram:PostalTradeAddress/ram:LineOne"/></cbc:StreetName>
					</xsl:if>
					<xsl:if test="string(ram:PostalTradeAddress/ram:LineTwo)">					
						<cbc:AdditionalStreetName><xsl:value-of select="ram:PostalTradeAddress/ram:LineTwo"/></cbc:AdditionalStreetName>
					</xsl:if>
					<xsl:if test="string(ram:PostalTradeAddress/ram:CityName)">					
						<cbc:CityName><xsl:value-of select="ram:PostalTradeAddress/ram:CityName"/></cbc:CityName>
					</xsl:if>
					<xsl:if test="string(ram:PostalTradeAddress/ram:PostcodeCode)">					
						<cbc:PostalZone><xsl:value-of select="ram:PostalTradeAddress/ram:PostcodeCode"/></cbc:PostalZone>
					</xsl:if>
					<xsl:if test="string(ram:PostalTradeAddress/ram:CountrySubDivisionName)">					
						<cbc:CountrySubentity><xsl:value-of select="ram:PostalTradeAddress/ram:CountrySubDivisionName"/></cbc:CountrySubentity>
					</xsl:if>
					<xsl:if test="string(ram:PostalTradeAddress/ram:LineThree)">
						<cac:AddressLine>
							<cbc:Line><xsl:value-of select="ram:PostalTradeAddress/ram:LineThree"/></cbc:Line>
						</cac:AddressLine>
					</xsl:if>
					<!--Mandatory is address class-->
					<cac:Country>
						<cbc:IdentificationCode><xsl:value-of select="ram:PostalTradeAddress/ram:CountryID"/></cbc:IdentificationCode>
					</cac:Country>				
				</cac:PostalAddress>
				
				<xsl:if test="ram:SpecifiedTaxRegistration">
					<cac:PartyTaxScheme>
						<cbc:CompanyID><xsl:value-of select="ram:SpecifiedTaxRegistration[1]/ram:ID"/></cbc:CompanyID>
						<cac:TaxScheme>
							<cbc:ID>VAT</cbc:ID>
						</cac:TaxScheme>
					</cac:PartyTaxScheme>
						...
						<cbc:CompanyID>
							<xsl:if test="string(ram:SpecifiedLegalOrganization/ram:ID/@schemeID)">
								<xsl:attribute name="schemeID">
									<xsl:value-of select="ram:SpecifiedLegalOrganization/ram:ID/@schemeID"/>
								</xsl:attribute>
							</xsl:if>
							<xsl:value-of select="ram:SpecifiedLegalOrganization/ram:ID"/>
						</cbc:CompanyID>
						<xsl:if test="ram:Description">
							<cbc:CompanyLegalForm><xsl:value-of select="ram:Description"/></cbc:CompanyLegalForm>
						</xsl:if>
					...
					<xsl:if test="ram:DefinedTradeContact">
						<cac:Contact>
							<xsl:if test="ram:DefinedTradeContact/ram:PersonName">
								<cbc:Name><xsl:value-of select="ram:DefinedTradeContact/ram:PersonName"/></cbc:Name>
							</xsl:if>
							<xsl:if test="ram:DefinedTradeContact/ram:TelephoneUniversalCommunication/ram:CompleteNumber">
								<cbc:Telephone><xsl:value-of select="ram:DefinedTradeContact/ram:TelephoneUniversalCommunication/ram:CompleteNumber"/></cbc:Telephone>
							</xsl:if>
							<xsl:if test="ram:DefinedTradeContact/ram:EmailURIUniversalCommunication/ram:URIID">
								<cbc:ElectronicMail><xsl:value-of select="ram:DefinedTradeContact/ram:EmailURIUniversalCommunication/ram:URIID"/></cbc:ElectronicMail>
							</xsl:if>
						</cac:Contact>
					</xsl:if>
	...
	<!--Varibles for the supplier class-->
		<cac:TaxRepresentativeParty>
				...
				<xsl:if test="string(ram:SpecifiedTaxRegistration/ram:ID)">
					<cac:PartyTaxScheme>
						<cbc:CompanyID><xsl:value-of select="ram:SpecifiedTaxRegistration/ram:ID"/></cbc:CompanyID>
						<cac:TaxScheme>
							<cbc:ID>VAT</cbc:ID>
						</cac:TaxScheme>
					</cac:PartyTaxScheme>
	...
	<!--Varibles for the Delivery class-->
		<cac:Delivery>	
				...
				<xsl:if test="string(ram:ActualDeliverySupplyChainEvent/ram:OccurrenceDateTime/udt:DateTimeString)">
					<cbc:ActualDeliveryDate><xsl:value-of select="ram:ActualDeliverySupplyChainEvent/ram:OccurrenceDateTime/udt:DateTimeString"/></cbc:ActualDeliveryDate>
				</xsl:if>
	...
	<xsl:variable name="DueDate">
		...
		<cac:PaymentMeans>	
			<cbc:PaymentMeansCode><xsl:value-of select="ram:TypeCode"/></cbc:PaymentMeansCode>
			
			<xsl:if test="../../../rsm:ExchangedDocument/ram:TypeCode = '381'">
			
				<cbc:PaymentDueDate><xsl:value-of select="$DueDate"/> </cbc:PaymentDueDate>
			</xsl:if>
			
			<xsl:if test="string(../ram:PaymentReference)">
				<cbc:PaymentID><xsl:value-of select="../ram:PaymentReference"/></cbc:PaymentID>
			</xsl:if>
			
			<xsl:if test="ram:ApplicableTradeSettlementFinancialCard">
				<cac:CardAccount>
					<cbc:PrimaryAccountNumberID><xsl:value-of select="ram:ApplicableTradeSettlementFinancialCard/ram:ID"/></cbc:PrimaryAccountNumberID>
					...
					<xsl:if test="ram:ApplicableTradeSettlementFinancialCard/ram:CardholderName">
						<cbc:HolderName><xsl:value-of select="ram:ApplicableTradeSettlementFinancialCard/ram:CardholderName"/></cbc:HolderName>
					</xsl:if>
				</cac:CardAccount>
			</xsl:if>
			
			<xsl:if test="ram:PayeePartyCreditorFinancialAccount or ram:PayerPartyDebtorFinancialAccount">
				<cac:PayeeFinancialAccount>
					<cbc:ID>
						<xsl:choose>
							...
							<xsl:when test="ram:PayerPartyDebtorFinancialAccount/ram:IBANID">
					...
					<xsl:if test="ram:PayeePartyCreditorFinancialAccount/ram:AccountName">
						<cbc:Name><xsl:value-of select="ram:PayeePartyCreditorFinancialAccount/ram:AccountName"/></cbc:Name>
					</xsl:if>
					
					
					<xsl:if test="ram:PayerSpecifiedDebtorFinancialInstitution/ram:BICID">
						<cac:FinancialInstitutionBranch>
							<cbc:ID><xsl:value-of select="ram:PayerSpecifiedDebtorFinancialInstitution/ram:BICID"/></cbc:ID>
						</cac:FinancialInstitutionBranch>
					</xsl:if>
				</cac:PayeeFinancialAccount>
				
				<xsl:if test="../ram:SpecifiedTradePaymentTerms/ram:DirectDebitMandateID or ../ram:CreditorReferenceID ">
					<cac:PaymentMandate>
						<xsl:if test="../ram:SpecifiedTradePaymentTerms/ram:DirectDebitMandateID">
							<cbc:ID><xsl:value-of select="../ram:SpecifiedTradePaymentTerms/ram:DirectDebitMandateID"/></cbc:ID>
						</xsl:if>
						
						<xsl:if test="../ram:CreditorReferenceID">
							<cac:PayerFinancialAccount>
								<cbc:ID><xsl:value-of select="../ram:CreditorReferenceID"/></cbc:ID>
							</cac:PayerFinancialAccount>
						</xsl:if>	
					</cac:PaymentMandate>
				</xsl:if>
	...
	<xsl:template match="rsm:SupplyChainTradeTransaction/ram:ApplicableHeaderTradeSettlement/ram:SpecifiedTradePaymentTerms">
		<cac:PaymentTerms>	
			<cbc:Note><xsl:value-of select="ram:Description"/></cbc:Note>
		</cac:PaymentTerms>
	</xsl:template>
	...
	<xsl:template match="rsm:SupplyChainTradeTransaction/ram:ApplicableHeaderTradeSettlement/ram:SpecifiedTradeAllowanceCharge">
		...
		<cac:AllowanceCharge>	
			...
			<!--Mandatory-->
			<cbc:ChargeIndicator><xsl:value-of select="ram:ChargeIndicator/udt:Indicator"/></cbc:ChargeIndicator>
			
			<xsl:if test="ram:ReasonCode">
				<cbc:AllowanceChargeReasonCode><xsl:value-of select="ram:ReasonCode"/></cbc:AllowanceChargeReasonCode>
			</xsl:if>
			
						
			<xsl:choose>
				<xsl:when test="ram:Reason">
					<cbc:AllowanceChargeReason><xsl:value-of select="ram:Reason"/></cbc:AllowanceChargeReason>
				</xsl:when>
				<xsl:otherwise><xsl:value-of select="'n/a'"/></xsl:otherwise>
			</xsl:choose>
			
			<xsl:if test="ram:CalculationPercent">
				<cbc:MultiplierFactorNumeric><xsl:value-of select="ram:CalculationPercent"/></cbc:MultiplierFactorNumeric>
			</xsl:if>
			
			<!--Mandatory-->
			<cbc:Amount currencyID="{$HeaderCurrency}"><xsl:value-of select="format-number(ram:ActualAmount, '0.00')"/></cbc:Amount>
			
			<xsl:if test="ram:BasisAmount">
				<cbc:BaseAmount currencyID ="{$HeaderCurrency}"><xsl:value-of select="format-number(ram:BasisAmount, '0.00')"/></cbc:BaseAmount>
			</xsl:if>
	...
	<xsl:template match="rsm:SupplyChainTradeTransaction/ram:ApplicableHeaderTradeSettlement/ram:ApplicableTradeTax">
		...
		<cac:TaxSubtotal>	
			...
			<xsl:if test="ram:BasisAmount">
				<cbc:TaxableAmount currencyID="{$HeaderCurrency}"><xsl:value-of select="format-number(ram:BasisAmount, '0.00')"/></cbc:TaxableAmount>
			</xsl:if>
			...
			<!--Mandatory-->
            <cac:TaxCategory>
                <cbc:ID><xsl:value-of select="ram:CategoryCode"/></cbc:ID>
                <xsl:if test="ram:RateApplicablePercent">
                    <cbc:Percent><xsl:value-of select="ram:RateApplicablePercent"/></cbc:Percent>
                </xsl:if>
                <xsl:if test="ram:ExemptionReasonCode">
					<cbc:TaxExemptionReasonCode><xsl:value-of select="ram:ExemptionReasonCode"/></cbc:TaxExemptionReasonCode>
                </xsl:if>
                <xsl:if test="ram:ExemptionReason">
					<cbc:TaxExemptionReason><xsl:value-of select="ram:ExemptionReason"/></cbc:TaxExemptionReason>
                </xsl:if>
	...
	<xsl:template match="rsm:SupplyChainTradeTransaction/ram:ApplicableHeaderTradeSettlement/ram:SpecifiedTradeSettlementHeaderMonetarySummation">
		...
		<cac:LegalMonetaryTotal>
			<!--Mandatory-->	
			<cbc:LineExtensionAmount currencyID="{$HeaderCurrency}"><xsl:value-of select="format-number(ram:LineTotalAmount, '0.00')"/></cbc:LineExtensionAmount>
			<!--Mandatory-->
			<cbc:TaxExclusiveAmount currencyID="{$HeaderCurrency}"><xsl:value-of select="format-number(ram:TaxBasisTotalAmount, '0.00')"/></cbc:TaxExclusiveAmount>
			<!--Mandatory-->
			<cbc:TaxInclusiveAmount currencyID="{$HeaderCurrency}"><xsl:value-of select="format-number(ram:GrandTotalAmount, '0.00')"/></cbc:TaxInclusiveAmount>
			
			<xsl:if test="ram:AllowanceTotalAmount">
				<cbc:AllowanceTotalAmount currencyID="{$HeaderCurrency}"><xsl:value-of select="format-number(ram:AllowanceTotalAmount, '0.00')"/></cbc:AllowanceTotalAmount>
			</xsl:if>
			
			<xsl:if test="ram:ChargeTotalAmount">
				<cbc:ChargeTotalAmount currencyID="{$HeaderCurrency}"><xsl:value-of select="format-number(ram:ChargeTotalAmount, '0.00')"/></cbc:ChargeTotalAmount>
			</xsl:if>
			
			<xsl:if test="ram:TotalPrepaidAmount">
				<cbc:PrepaidAmount currencyID="{$HeaderCurrency}"><xsl:value-of select="format-number(ram:TotalPrepaidAmount, '0.00')"/></cbc:PrepaidAmount>
			</xsl:if>
			
			<xsl:if test="ram:RoundingAmount">
				<cbc:PayableRoundingAmount currencyID="{$HeaderCurrency}"><xsl:value-of select="ram:RoundingAmount"/></cbc:PayableRoundingAmount>
			</xsl:if>
			
			<!--Mandatory-->
			<cbc:PayableAmount currencyID="{$HeaderCurrency}"><xsl:value-of select="format-number(ram:DuePayableAmount, '0.00')"/></cbc:PayableAmount>
		</cac:LegalMonetaryTotal>
	...
	<xsl:template match="rsm:SupplyChainTradeTransaction/ram:IncludedSupplyChainTradeLineItem">
		...
		<cac:InvoiceLine>
			...
			<!--Mandatory-->
			<cbc:ID><xsl:value-of select="ram:AssociatedDocumentLineDocument/ram:LineID"/></cbc:ID>
			
			<xsl:if test="ram:AssociatedDocumentLineDocument/ram:IncludedNote/ram:Content">
			 <cbc:Note><xsl:value-of select="ram:AssociatedDocumentLineDocument/ram:IncludedNote/ram:Content"/></cbc:Note>
			</xsl:if>
			
			<!--Mandatory-->
			<cbc:InvoicedQuantity unitCode="{$UnitCodeScheme}"><xsl:value-of select="ram:SpecifiedLineTradeDelivery/ram:BilledQuantity"/></cbc:InvoicedQuantity>
			
			<!--Mandatory-->
			<cbc:LineExtensionAmount currencyID="{$HeaderCurrency}"><xsl:value-of select="format-number(ram:SpecifiedLineTradeSettlement/ram:SpecifiedTradeSettlementLineMonetarySummation/ram:LineTotalAmount, '0.00')"/></cbc:LineExtensionAmount>
			
			<xsl:if test="ram:SpecifiedLineTradeSettlement/ram:ReceivableSpecifiedTradeAccountingAccount/ram:ID">
				<cbc:AccountingCost><xsl:value-of select="ram:SpecifiedLineTradeSettlement/ram:ReceivableSpecifiedTradeAccountingAccount/ram:ID"/></cbc:AccountingCost>
			</xsl:if>
			
			<!--Inserting Invoice period if present-->
			<xsl:if test="ram:SpecifiedLineTradeSettlement/ram:BillingSpecifiedPeriod">
				<cac:InvoicePeriod>
					<cbc:StartDate><xsl:value-of select="substring(ram:SpecifiedLineTradeSettlement/ram:BillingSpecifiedPeriod/ram:StartDateTime/udt:DateTimeString, 1,4)"/>-<xsl:value-of select="substring(ram:SpecifiedLineTradeSettlement/ram:BillingSpecifiedPeriod/ram:StartDateTime/udt:DateTimeString, 5,2)"/>-<xsl:value-of select="substring(ram:SpecifiedLineTradeSettlement/ram:BillingSpecifiedPeriod/ram:StartDateTime/udt:DateTimeString, 7,2)"/>
					</cbc:StartDate>
					<cbc:EndDate>
	...
	<xsl:template match="ram:BuyerOrderReferencedDocument">
		<cac:OrderLineReference>
			<cbc:LineID><xsl:value-of select="ram:LineID"/></cbc:LineID>
		</cac:OrderLineReference>
	</xsl:template>
	...
	<xsl:template match="ram:SpecifiedLineTradeSettlement/ram:AdditionalReferencedDocument">
		<cac:DocumentReference>
			...
			<xsl:if test="ram:TypeCode">
				<cbc:DocumentTypeCode><xsl:value-of select="ram:TypeCode"/></cbc:DocumentTypeCode>
			</xsl:if>
		</cac:DocumentReference>
	</xsl:template>
	...
	<xsl:template match="ram:SpecifiedTradeProduct">
		<cac:Item>	
			...
			<xsl:if test="ram:Description">
				<cbc:Description><xsl:value-of select="ram:Description"/></cbc:Description>
			</xsl:if>
	...
	<xsl:template match="ram:BuyerAssignedID">
		<cac:BuyersItemIdentification>
			<cbc:ID><xsl:value-of select="../ram:BuyerAssignedID"/></cbc:ID>
		</cac:BuyersItemIdentification>
	</xsl:template>
	
	<xsl:template match="ram:SellerAssignedID">
		<cac:SellersItemIdentification>
			<cbc:ID><xsl:value-of select="../ram:SellerAssignedID"/></cbc:ID>
		</cac:SellersItemIdentification>
	</xsl:template>
	...
	<xsl:template match="ram:GlobalID">
		<cac:StandardItemIdentification>
			<cbc:ID>
					<xsl:attribute name="schemeID" select="'0160'"></xsl:attribute>
			<xsl:value-of select="../ram:GlobalID"/></cbc:ID>
		</cac:StandardItemIdentification>
	</xsl:template>
	
	<xsl:template match="ram:OriginTradeCountry">
		<cac:OriginCountry>
			<cbc:IdentificationCode><xsl:value-of select="ram:ID"/></cbc:IdentificationCode>
		</cac:OriginCountry>
	</xsl:template>
			...
			<xsl:if test="ram:ClassCode">
				<cbc:ItemClassificationCode>
					<xsl:if test="ram:ClassCode/@listID">
						<xsl:attribute name="listID" select="'ZZZ'"></xsl:attribute>
					</xsl:if>
					<xsl:if test="ram:ClassCode/@listVersionID">
						<xsl:attribute name="listVersionID" select="ram:ClassCode/@listVersionID"></xsl:attribute>
					</xsl:if>
					<xsl:value-of select="ram:ClassCode"/></cbc:ItemClassificationCode>
			</xsl:if>
		</cac:CommodityClassification>
	</xsl:template>
	...
	<xsl:template match="ram:SpecifiedTradeProduct/ram:ApplicableProductCharacteristic">
		<cac:AdditionalItemProperty>
			<cbc:Name><xsl:value-of select="ram:Description"/></cbc:Name>
			<cbc:Value><xsl:value-of select="ram:Value"/></cbc:Value>
		</cac:AdditionalItemProperty>
	</xsl:template>
	<!--End Item templates-->
	...
	<xsl:template match="ram:SpecifiedLineTradeAgreement">
		<cac:Price>
			<cbc:PriceAmount currencyID="{$HeaderCurrency}"><xsl:value-of select="ram:NetPriceProductTradePrice/ram:ChargeAmount"/></cbc:PriceAmount>
		...
		<cac:AllowanceCharge>
			<cbc:ChargeIndicator><xsl:value-of select="'false'"/></cbc:ChargeIndicator>
			<cbc:Amount currencyID="{$HeaderCurrency}"><xsl:value-of select="ram:GrossPriceProductTradePrice/ram:AppliedTradeAllowanceCharge/ram:ActualAmount"/></cbc:Amount>
		</cac:AllowanceCharge>
		</xsl:if>
	...
	<xsl:variable name="CustomizationID" select="'urn:cen.eu:en16931:2017:extended:urn:fdc:peppol.eu:2017:poacc:billing:3.0'"/>
	<xsl:variable name="ProfileID" select="'urn:fdc:peppol.eu:2017:poacc:billing:01:1.0'"/>
	<xsl:variable name="InvoiceNumber" select="rsm:ExchangedDocument/ram:ID"/>
	...
	<xsl:variable name="CurrencyCode" select="rsm:SupplyChainTradeTransaction/ram:ApplicableHeaderTradeSettlement/ram:InvoiceCurrencyCode"/>
	<xsl:variable name="TaxCurrencyCode" select="rsm:SupplyChainTradeTransaction/ram:ApplicableHeaderTradeSettlement/ram:InvoiceCurrencyCode"/>
	<xsl:variable name="AccountingCost" select="rsm:SupplyChainTradeTransaction/ram:ApplicableHeaderTradeSettlement/ram:ReceivableSpecifiedTradeAccountingAccount/ram:ID"/>
	<xsl:variable name="BuyerReference" select="rsm:SupplyChainTradeTransaction/ram:ApplicableHeaderTradeAgreement/ram:BuyerReference"/>
	...
	<xsl:template name="CreditNote">
		...
		<cac:CreditNoteLine>
			...
			<!--Mandatory-->
			<cbc:CreditedQuantity unitCode="{$UnitCodeScheme}"><xsl:value-of select="ram:SpecifiedLineTradeDelivery/ram:BilledQuantity"/></cbc:CreditedQuantity>
			...
			<xsl:if test="ram:SpecifiedLineTradeSettlement/ram:BillingSpecifiedPeriod">
				<cac:InvoicePeriod>
					<cbc:StartDate><xsl:value-of select="ram:SpecifiedLineTradeSettlement/ram:BillingSpecifiedPeriod/ram:StartDateTime/udt:DateTimeString"/></cbc:StartDate>
					<cbc:EndDate><xsl:value-of select="ram:SpecifiedLineTradeSettlement/ram:BillingSpecifiedPeriod/ram:EndDateTime/udt:DateTimeString"/></cbc:EndDate>
				</cac:InvoicePeriod>
			</xsl:if>
 ...
 <!--Conversion templates for mapping values betweeen CII and BIS in relations to codes and identifiers-->
	...
	<xsl:template name="EndpointIDSchemeIDmapping">
		<xsl:param name="GlobalIDScheme"></xsl:param>
		<xsl:param name="GlobalIDSchemeValue"></xsl:param>
	...
	<!--Build the Endpoint element-->
		<cbc:EndpointID schemeID ="{$GlobalIDScheme}"><xsl:value-of select="$GlobalIDSchemeValue"/></cbc:EndpointID>
	</xsl:template>
	...
	<xsl:template name="DeliverySchemeIDmapping">
		...
		<cbc:ID schemeID ="{$GlobalIDScheme}"><xsl:value-of select="$GlobalIDSchemeValue"/></cbc:ID>
