...
public class SpecificationUtil {
    ...
    public List<Predicate> generateSpecificationPredicates(
        ...
        fieldsFromEntityModel.addAll(Arrays.asList(entityClass.getSuperclass().getSuperclass().getDeclaredFields()));
        for (Field field : fieldsFromEntityModel) {
            if (Modifier.isPrivate(field.getModifiers())) {
                String parameter = request.getParameter(field.getName());
                if (Objects.isNull(parameter) || Objects.equals(parameter, "ALL")) {
                    continue;
                }
